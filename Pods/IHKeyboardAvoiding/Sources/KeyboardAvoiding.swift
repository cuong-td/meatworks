...
@objc public class KeyboardAvoiding: NSObject {
    ...
    public static var avoidingBlock: ((Bool, CGFloat, CGFloat, UIView.AnimationOptions)->Void)? {
        willSet {
            self.initialise()
        }
        didSet {
    ...
    @objc class func didChange(_ notification: Foundation.Notification) {
        ...
        let keyboardFrameBegin = notification.userInfo![UIResponder.keyboardFrameBeginUserInfoKey] as! CGRect
        ...
        if isKeyBoardShowing {
            for triggerView in self.triggerViews {
                ...
                if self.avoidingBlock != nil {
                    self.avoidingBlock!(isKeyBoardShowing, animationDuration, keyboardFrame.size.height, UIView.AnimationOptions(rawValue: UInt(animationOptions)))
                }
            }
    ...
    public class func removeTriggerView(_ triggerView: UIView) {
        if let index = triggerViews.firstIndex(of: triggerView) as Int? {
    ...
    private class func initialise() {
        ...
        if self.avoidingBlock == nil && self.avoidingView == nil {
            NotificationCenter.default.addObserver(self, selector: #selector(KeyboardAvoiding.applicationDidEnterBackground(_:)), name: UIApplication.didEnterBackgroundNotification, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(KeyboardAvoiding.didChange(_:)), name: UIResponder.keyboardWillChangeFrameNotification, object: nil)
        }
    }
